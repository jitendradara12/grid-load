...
if npp_df.empty:
    ...
    combined = pd.concat([iced_df_filtered, npp_df_filtered]).sort_values('datetime').reset_index(drop=True)
    
    combined['value'] = combined['value'].interpolate(method='linear').ffill().bfill()
    
    print(f"Combined time series spans from {combined['datetime'].min()} to {combined['datetime'].max()}.")
